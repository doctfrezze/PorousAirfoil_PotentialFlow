...
def extrapolate_matrices(CL_matrix, CD_matrix, NumPan_list):
    ...
    for i in range(num_aoa):
        ...
        F_cl,h, *_ = RICHARDSON_EXTRAPOLATION(NumPan_list, cl_values)
        F_cd,h, *_ = RICHARDSON_EXTRAPOLATION(NumPan_list, cd_values)

        extrapolated_CL[i] = F_cl[0]
        extrapolated_CD[i] = F_cd[0]

    return extrapolated_CL, extrapolated_CD,h
...
def RICHARDSON_EXTRAPOLATION(NumPan_list,Result_list):
    ...
    F[1:] = Result_list[::-1]
    h[1] = NumPan_list[-1]/NumPan_list[-1]
    h[2] = NumPan_list[-1]/NumPan_list[-2]
    h[3] = NumPan_list[-1]/NumPan_list[-3]
...
def find_p(r21, r32, f1, f2, f3):
    ...
    sol = root_scalar(equation_p, args=(r21, r32, f1, f2, f3), method='brentq', bracket=[0.1, 10])
    if sol.converged:
        return sol.root
    else:
